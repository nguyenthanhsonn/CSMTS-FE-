    ...
    >
          ...
          <tbody>
            {paginatedData.map((row, rowIdx) => (
              <tr
                ...
                className={`hover:bg-[#F8F9FA] transition duration-150 ${onRowClick ? 'cursor-pointer' : ''}`}
              >
                {columns.map((col, colIdx) => {
                  const cellValue = col.key !== 'actions' ? row[col.key as keyof T] : undefined;
                  return (
                    <td key={colIdx} className="py-4 px-4 text-[#1A1B1E] max-w-[300px] truncate border-b border-[#E9ECEF]">
              ...
              Array.from({ length: emptyRowCount }).map((_, i) => (
                <tr key={`empty-${i}`}>
                  {columns.map((_, colIdx) => (
                    <td key={colIdx} className="px-4 py-4 border-b border-[#E9ECEF]/60">
                      &nbsp;
                    </td>
                  ))}
                </tr>
      ...
      {data.length > 0 && (
        <div className="px-4 py-3 border-t border-gray-50 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mt-auto">
          ...
          <div className={`flex items-center gap-2.5 self-center ${paginationAlign === 'left' ? 'sm:mr-auto' : showSummary ? 'sm:self-auto' : 'sm:ml-auto'}`}>
            <button
